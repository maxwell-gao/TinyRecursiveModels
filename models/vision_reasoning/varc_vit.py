...
class VARCViTConfig(BaseModel):
    ...
    seq_len: int
    puzzle_emb_ndim: int = (
        0  # Not used by ARCViT directly but required by pretrain.py logic
    )
...
class VARCViTInner(nn.Module):
    def __init__(self, config: VARCViTConfig) -> None:
        ...
        if detected_image_size * detected_image_size != self.config.seq_len:
            ...
            if self.config.image_size * self.config.image_size != self.config.seq_len:
                print(
                    f"Warning: seq_len {self.config.seq_len} is not a perfect square and doesn't match image_size {self.config.image_size} squared."
                )
        else:
            if self.config.image_size != detected_image_size:
                print(
                    f"Overriding image_size {self.config.image_size} with detected size {detected_image_size} from seq_len {self.config.seq_len}"
                )
        ...
        if self.config.image_size % self.config.patch_size != 0:
            print(
                f"Warning: image_size {self.config.image_size} is not divisible by patch_size {self.config.patch_size}. Falling back to patch_size=1."
            )
    ...
    def forward(
        self, carry: VARCViTInnerCarry, batch: Dict[str, torch.Tensor]
    ) -> Tuple[VARCViTInnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        q_halt_logits = torch.full(
            (B,), 10.0, device=inputs.device, dtype=torch.float32
        )
        q_continue_logits = torch.full(
            (B,), -10.0, device=inputs.device, dtype=torch.float32
        )
...
class VARCViT(nn.Module):
    ...
    def initial_carry(self, batch: Dict[str, torch.Tensor]):
        ...
        return VARCViTCarry(
            ...
            steps=torch.zeros(
                (batch_size,), dtype=torch.int32, device=batch["inputs"].device
            ),
            halted=torch.zeros(
    ...
    ) -> Tuple[VARCViTCarry, Dict[str, torch.Tensor]]:
