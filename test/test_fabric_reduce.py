...
def test_inplace():
    ...
    if t.data_ptr() != result.data_ptr():
        print("Conclusion: Fabric returned a new Tensor! The original Tensor was not modified in-place (or should not be relied upon).")
        print("Must write as: t = fabric.all_reduce(t)")
    else:
        print("Conclusion: It was modified in-place? (This might be special on single-device CPU, but usually not in distributed settings)")
